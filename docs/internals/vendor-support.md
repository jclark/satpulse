...
- **JSON field names match the vendor doc.** Each decode struct field must marshal under exactly the name the vendor's reference guide gives it, so the JSON emitted by `gps/gpsdecode`/`internal/decodecmd` is directly cross-referenceable against the guide's message tables. Where the guide's own name is already a valid exported Go identifier, use it as the field name and no tag is needed (model: `gps/lib/sbfbin`, whose guide names fields `TOW`, `WNc`, `RxClkBias`). Where it is not -- a lowercase initial, or characters Go does not allow -- add a JSON tag carrying the guide's spelling (model: `gps/lib/ubxbin`, whose guide names fields `iTOW`, `gSpeed`).
