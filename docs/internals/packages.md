...
`cmd/satpulsetui` provides main for satpulsetui, a terminal UI counterpart to SatPulse Workbench: a Bubble Tea (v2) shell over `gps/app/session`, used in process with no HTTP or serialization layer. Its `Sink` queues session events into the Bubble Tea message loop without blocking, coalescing keep-latest per event name for the high-rate snapshot events and bounding the pending packet stream; slog is routed through `session.NewLogHandler` with a discard base handler so nothing writes to the terminal behind Bubble Tea's back. Full-screen views mirror the workbench tabs -- Monitor (summary, clock, PVT tables, satellite signal bars, survey, position statistics), Packets (scrolling list with decode detail, gated on visibility via `Wants`), Corrections, Configuration (gated on the probed `ConfigSupport` flags), and Messages -- with a connection form as an overlay in place of the workbench's connection bar.
