...
The client supports only the UDP transport and two-step mode, which matches the default for Linux PTP. However, it requires the `currentOffsetUtcValid` flags  to be 1 in the grandmaster settings. (Without this, my Windows machine was very precisely synchronized to be 37 seconds off.) SatPulse will take care of this automatically if you configure the `[ptp]` table
