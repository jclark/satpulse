...
- **Single source of protocol-format truth.** This package owns *all* knowledge of the message encoding. Every wire/text value a higher layer needs -- message IDs, enum codes, unavailable-data sentinels, bitfield masks/shifts, field offsets, command/response classifiers -- is defined here as an exported named constant, type, or method. The higher layers (`gps/internal/<vendor>`, config, RINEX, `gps/msgfile`) must contain no magic protocol constants; they reference this package's exported constants only. If a higher layer turns out to need a raw value that isn't exported yet, export it here rather than inlining a literal downstream.
