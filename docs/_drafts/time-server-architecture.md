...
In can be hard to understand how everything fits together with a PTP/NTP time server. In this post, I want to explain how things work when using SatPulse.
...
- System clock. There is an underlying raw hardware counter. The kernel exposes three kinds of clock based on this.
   ...
   - the realtime clock is adjusted by NTP for frequency and phase, so that it corresponds closely to UTC
...
- Reading timestamp events from the PHC. The PHC hardware can record the time of the PHC at which a time pulse occurs on a PHC SDP. The GPS emits time pulses that are precisely aligned with the beginning of a second. So if the timestamp says the pulse is 45 nanoseconds after the beginning of the second, you know that the PHC is different from the true time by 45 nanoseconds plus some integral number of seconds.
- Reading messages from the GPS receiver over the serial port. These messages give the approximate current time. By correlating these messages with the timestamp events, you can determine which second the pulse is marking the beginning of, and thus know exactly how much the PHC is off from the true time.
- Adjusting the PHC. From the timestamp events and GPS messages, you can derive a measurement once per second saying how much the PHC is off from the true time. When the process starts up, you use this to set the PHC time. After that you use it to continually adjust the frequency of the PHC to keep the difference between the PHC and true time as small as possible. Note that setting the PHC time is relatively imprecise, because this is involves separate operations to read and write the time. To get the PHC time precisely right, you have to adjust the frequency.

The PTP standard requires that when time is transferred in from an external source, certain metadata relating to the time is also transferred in. With ptp4l, this metadata is provided using a specific PTP management message defined by ptp4l (called GRANDMASTER_SETTINGS_NP, NP for non-portable). Accordingly satpulsed implements a PTP management client and  uses it to send GRANDMASTER_SETTINGS_NP messages to ptp4l over a socket. We can divide the kinds of metadata that PTP requires into two.

The first kind of metadata relates to the offset between TAI time and UTC time. The time of the PHC uses the PTP timescale which is based on TAI. TAI differs from UTC time by an integral number of seconds, which changes when leap seconds occur. It turns out that all GNSS systems (other than GLONASS) work in a similar way: their clocks use a timescale based on TAI, but they also provide metadata about the offset between TAI and UTC time. Usually GPS receivers hide all this complexity and output messages with UTC time, and this is what you want with NTP. But for PTP what you want is for the GPS receiver to output messages with TAI time (or rather GNSS time, which is a fixed number of seconds from TAI time) and also messages about leap seconds. During start-up, satpulsed sends configuration messages to the GPS to make it do this, and then it uses these messages to provide the appropriate metadata to ptp4l.

The second kind of metadata relates to the clock quality. This says whether the PHC is synchronized to GNSS at all, and if it is, how accurately it is synchronized. A PTP server broadcasts the quality of its clock. PTP clients use this information to choose the best PTP server. So if there's a problem with the GPS receiver which prevents satpulsed from keeping the PHC synchronized with the GPS receiver time, then satpulsed needs to tell ptp4l about this promptly, so that ptp4l can change the metadata that it tells clients about its clock quality. This will allow clients to switch over to an alternative PTP server. This also implies that satpulsed has to monitor the synchronization state.

When used with chrony, satpulsed in addition takes simultaneous readings of the PHC and the realtime clock. When the PHC is in sync with the GPS receiver time, then these readings can be used to generate a sample saying how much the realtime clock is off from the true time; satpulsed will send these samples to chrony over a socket using the refclock SOCK protocol. Chrony can use this as a reference clock.

## Comparison to classic NTP server

I want to highlight how this architecture differs from a classic NTP server, using an NTP server such as ntpd or chrony together with gpsd. Apart from the addition of a PTP implementation, the key difference is in the presence of a PHC. In a classic NTP architecture, the GPS receiver is attached to a serial port pin or GPIO pin. When a GPS receiver emits a pulse, an interrupt will occur and the kernel will record the time of the pulse. The kernel makes these timestamps available to applications via a PPS kernel device, which will be read by either gpsd or the NTP server.
