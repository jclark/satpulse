...
- `satpulsetool` has a new `serial` command, which lists serial ports, detects the speed of a specified GNSS receiver from its output, or scans all enumerated ports in parallel. Enumeration does not open the ports and supports JSON Lines output; detection can record wrong-speed input in a packet log. (#326, #394)
