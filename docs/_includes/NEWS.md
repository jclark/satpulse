...
- Septentrio receivers now support high-level configuration: probing and `--show-receiver`/`--show-config`, GNSS and signal selection, time pulse, minimum elevation, antenna cable delay, OSNMA navigation-message authentication, static and fixed-position modes, NMEA/binary/raw/RTCM output control, and save/reload/reset/factory-reset, through both `satpulsetool gps` and `satpulsed` auto-configuration. Galileo HAS PPP is enabled automatically when the Galileo E6 signal is enabled on receivers with that capability. (#341)
