...
- `satpulsetool decode` and `satpulsetool annotate` have a new `--vendor` option, which decodes NovAtel-format packets using that vendor's variant of the protocol; SatPulse Workbench uses the vendor it was given in the same way.
