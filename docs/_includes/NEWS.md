...
- `satpulsed` can now run as a Windows service: `satpulsed --install` registers it with the Service Control Manager so that it starts automatically at boot, with the daemon log written to the file given by `--log-file` and service lifecycle events recorded in the Windows Event Log; `satpulsed --uninstall` removes the service. (#338)
