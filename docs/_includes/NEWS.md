...
- Allystar receivers (tested on the TAU1201, TAU1302, and TAU951M-P200) now support high-level configuration with `satpulsetool gps` and `satpulsed`: probing and receiver identification, NMEA sentence selection (`--nmea-out`), PVT and satellite message output (`--pvt-out`, `--sats-out`), RTCM MSM4/MSM7/ARP output on receivers that have it (`--rtcm-out`), raw data output (`--raw-out`), time pulse configuration (`--pps`), minimum elevation (`--min-elev`), survey-in and fixed position (`--survey`, `--fixed-pos-ecef`, `--fixed-pos-llh`, `--mobile`), GNSS signal selection (`--gnss`, `--band`, `--signal`), serial speed (`--speed`), and non-volatile memory operations (`--save`, `--save-all`, `--reload`, `--reset`, `--factory-reset`). (#344)
