...
- High-level configuration now supports CASIC (Zhongke Microelectronics) receivers, covering both the V5 (URANUS5) and V6 (URANUS6) firmware families: probing, NMEA and binary message output (including raw measurement, RTCM, leap second, and survey progress messages), time pulse, time mode and survey-in, GNSS signal selection, minimum elevation, antenna cable delay, save/reload/reset, and baud rate. `satpulsetool gps` and `satpulsed` can now configure receivers such as the ATGM332D and AT632. (#229)
