...
- SatPulse no longer restores the original serial speed or other attributes affecting the UART hardware when closing a serial port. This avoids unnecessary hardware changes when the port is reopened, minimizing the risk of serial corruption on Raspberry Pi UARTs.
