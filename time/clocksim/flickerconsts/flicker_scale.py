...
Usage:
    python flicker_scale.py --tau0 10.0 --tau-max 1000.0 --ratio 2.0

This can be integrated into Go builds using `go generate`:
    //go:generate python flicker_scale.py --tau0 10.0 --tau-max 1000.0 --ratio 2.0
"""
...
# Empirical fudge factor to account for discrete-time effects (dt=1s sampling,
# Nyquist at 0.5 Hz) and Kasdin-Walter approximation errors.
#
...
def compute_kasdin_walter_allan_scale(
    ...
    return correction_factor_theoretical
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Prefix for generated constant names (default: empty)",
    )
    parser.add_argument(
        "--output",
        type=str,
        help="Output file path for Go constants (only used with --go-const)",
    )
    parser.add_argument(
        "--fudge-factor",
        type=float,
        default=FLICKER_FUDGE_FACTOR,
        help=f"Empirical fudge factor (default: {FLICKER_FUDGE_FACTOR})",
    )
    ...
    if args.go_const:
        ...
        output_lines = [
            ...
            "",
            f"const {args.prefix}Tau0 = {args.tau0}",
            f"const {args.prefix}TauMax = {args.tau_max}",
            f"const {args.prefix}Ratio = {args.ratio}",
            "",
            "// Allan-weighted analytical scale factor for Kasdin-Walter filter.",
            f"const {args.prefix}AnalyticScaleFactor = {scale_factor}",
            "",
            "// Empirical fudge factor for discrete-time effects and approximation errors.",
            "// Calibrated from simulator validation for τ range 10-1000s, ratio=2.0.",
            f"const {args.prefix}FudgeFactor = {args.fudge_factor}",
            "",
            "// Combined scaling factor: analytic × fudge.",
            f"const {args.prefix}ScaleFactor = "
            f"{args.prefix}AnalyticScaleFactor * {args.prefix}FudgeFactor",
        ]
        output_text = "\n".join(output_lines) + "\n"
        ...
        print(f"\nAnalytic scaling factor: {scale_factor:.6f}")
        print(f"Fudge factor:            {args.fudge_factor:.6f}")
        combined = scale_factor * args.fudge_factor
        print(f"Combined:                {combined:.6f}")
        print("\nUsage in Go:")
        print(f"  scale := stddevPpb / 1e9 / math.Sqrt({len(tau_list)}) * {combined:.6f}")
