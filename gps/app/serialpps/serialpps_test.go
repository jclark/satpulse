...
func TestDetectMethodSelection(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var logs bytes.Buffer
			lg := slog.New(slog.NewTextHandler(&logs, &slog.HandlerOptions{Level: slog.LevelDebug}))
			err := Detect(ctx, lg, w, Wiring{Pin: gpsio.ModemCTS}, Config{Method: tc.method}, make(chan CandidateEdge, 1), stats, nil)
...
func TestDetectWithoutWaiterPolls(t *testing.T) {
	...
	var logs bytes.Buffer
	lg := slog.New(slog.NewTextHandler(&logs, &slog.HandlerOptions{Level: slog.LevelDebug}))
	err := Detect(ctx, lg, &testPoller{cancel: cancel}, Wiring{Pin: gpsio.ModemCTS}, Config{}, make(chan CandidateEdge, 1), stats, nil)
...
func TestDetectForcedWaitWithoutWaiter(t *testing.T) {
	for _, method := range []gpsio.PPSMethod{gpsio.PPSMethodWait, gpsio.PPSMethodKernel} {
		t.Run(method.String(), func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()
			err := Detect(ctx, testLog, &testPoller{cancel: cancel}, Wiring{Pin: gpsio.ModemCTS}, Config{Method: method}, make(chan CandidateEdge, 1), nil, nil)
