...
func TestPollStatsSummary(t *testing.T) {
	...
	stats.addWindow(goodCatch, false)
	stats.addWindow(goodCatch, true)
...
func TestPollStatsLog(t *testing.T) {
	...
	for _, want := range []string{
		`msg="serial PPS polling statistics" acquire.windows=1 acquire.edges=1 acquire.rejected=0 track.windows=0 track.edges=0 track.rejected=0`,
