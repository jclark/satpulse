...
type Options struct {
	...
	// OmitZeroDo omits RTCM MSM Doppler observations with numeric value
...
func (c *Converter) convertCell(t rinex.Time, gnss rtcmbin.GNSS, m *rtcmbin.MSMHiRes, satIndex, cellIndex int, satID, sigID uint8) (bool, error) {
	...
	if dop, ok := doppler(m, satIndex, cellIndex, freq, hasFrequency, c.opts.UseSpecPhaseRangeRateSign); ok && (!c.opts.OmitZeroDo || dop != 0) {
		obs.Do = opt.Make(dop)
	}
...
func frequencyHz(sys, sig string, frq *int8) (float64, bool) {
	return rinex.SignalFrequencyHz(sys, rinex.SignalID(sig), frq)
}
