...
func TestLeapTim2Ls(t *testing.T) {
	tRead := time.Date(2026, 6, 12, 0, 0, 0, 0, time.UTC)
	tests := []struct {
		name   string
		m      casbin.Tim2Ls
		expect *gpsprot.LeapSecondMsg
	}{
		{
			// The 2016-12-31 leap second: GPS week 1929 (137 mod 256),
			// day 7 (Saturday), resolved by walking back 256-week
			// steps from 2026 until the date is a quarter end.
			name: "past leap second resolved across the 256-week wrap",
			m: casbin.Tim2Ls{RaimType: casbin.Tim2RaimLsNormal, GNSSID: casbin.GPS,
				Wnlsf: 137, Dn: 7, Dtls: 17, Dtlsf: 18},
			expect: &gpsprot.LeapSecondMsg{
				LeapSecond: ptime.LeapSecondOnDate(
					time.Date(2016, 12, 31, 0, 0, 0, 0, time.UTC), 36, 37),
				GNSS: gpsprot.GPS,
			},
		},
		{
			name:   "no leap second info",
			m:      casbin.Tim2Ls{RaimType: casbin.Tim2RaimNoLsInfo},
			expect: nil,
		},
		{
			name: "invalid day number",
			m: casbin.Tim2Ls{RaimType: casbin.Tim2RaimLsNormal,
				Wnlsf: 137, Dn: 0, Dtls: 17, Dtlsf: 18},
			expect: nil,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := leapTim2Ls(&tc.m, tRead)
