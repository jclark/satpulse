...
func TestVersionConfigSupport(t *testing.T) {
	...
	bandSig := gpsprot.ConfigSupportBand | gpsprot.ConfigSupportSignal
	tests := []struct {
		name string
		ver  Version
		...
		{"F9P", testVers.f9p, bandSig | tmode2 | raw | msm},
		{"F9T before MSM4", testVers.f9t, bandSig | tmode2 | raw | gpsprot.ConfigSupportRTCMMSM7},
		{"F9T with MSM4", testVers.f9t25, bandSig | tmode2 | raw | msm | gpsprot.ConfigSupportRTCMBaseID},
		{"F10S", testVers.f10s, bandSig | gpsprot.ConfigSupportSpeed},
		{"X20P", testVers.x20p, bandSig | tmode2 | raw | msm | gpsprot.ConfigSupportRTCMBaseID},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
