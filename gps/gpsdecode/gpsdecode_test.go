...
func TestDecodeUBX(t *testing.T) {
	...
	pf, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, nil)
	if err != nil {
...
func TestDecodeUBXCfgValget(t *testing.T) {
	...
	_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Payload == nil {
...
func TestDecodeUBXCfgValgetRequest(t *testing.T) {
	...
	_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, true, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.CfgData == nil {
...
func TestDecodeUBXUnknown(t *testing.T) {
	...
	_, _, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, nil)
...
func TestDecodeNovAtelVendor(t *testing.T) {
	// BESTPOSB from a UM980, whose port encoding differs from OEM7's
	packet := mustHexDecode(
		"aa44121c2a00000348008c4459a06c09c8306420951200001f00120000000000100000" +
			"00a4744542af762b407b215e5443295940000098ec0ef62940a8a5f6c13d0000007e0f" +
			"d6402862b9401a05b84000000000000000000000803f1a12120081121150f5878993",
	)
	common := novmsg.CommonHdr{
		Sequence:           17548,
		IdleTime:           89,
		TimeStatus:         novmsg.TimeStatusFine,
		Week:               2412,
		MillisecondsOfWeek: 543437000,
		RecvStatus:         4757,
		Reserved:           31,
		Version:            18,
	}
	tests := []struct {
		...
		{"no vendor", nil, novmsg.MsgHdr[novmsg.Port]{Port: 3, CommonHdr: common}},
		{"Unicore", []gpsreg.Vendor{gpsreg.VendorUnicore}, novmsg.MsgHdr[novmsg.UnicorePort]{Port: novmsg.UnicoreCOM3, CommonHdr: common}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, tc.vendors)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if !reflect.DeepEqual(result.Header, tc.expect) {
				t.Errorf("got  %+v\nwant %+v", result.Header, tc.expect)
			}
		})
	}
}
...
func TestDecodeErrInvalidPacket(t *testing.T) {
	...
	_, _, err := Decode(gpsreg.CreatePacketFormats(nil), []byte("random data"), false, nil)
...
func TestDecodeChecksumError(t *testing.T) {
	...
	pf, _, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, nil)
	if pf == nil {
