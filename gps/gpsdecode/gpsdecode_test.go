...
func TestDecodeUBX(t *testing.T) {
	...
	pf, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, 0)
	if err != nil {
...
func TestDecodeUBXCfgValget(t *testing.T) {
	...
	_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Payload == nil {
...
func TestDecodeUBXCfgValgetRequest(t *testing.T) {
	...
	_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, true, 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.CfgData == nil {
...
func TestDecodeUBXUnknown(t *testing.T) {
	...
	_, _, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, 0)
...
func TestDecodeNovAtelVendor(t *testing.T) {
	...
	tests := []struct {
		...
		{"no vendor", 0, novmsg.MsgHdr[novmsg.Port]{Port: 3, CommonHdr: common}},
		{"Unicore", gpsreg.VendorUnicore, novmsg.MsgHdr[novmsg.UnicorePort]{Port: novmsg.UnicoreCOM3, CommonHdr: common}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			_, result, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, tc.vendor)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestDecodeErrInvalidPacket(t *testing.T) {
	...
	_, _, err := Decode(gpsreg.CreatePacketFormats(nil), []byte("random data"), false, 0)
...
func TestDecodeChecksumError(t *testing.T) {
	...
	pf, _, err := Decode(gpsreg.CreatePacketFormats(nil), packet, false, 0)
	if pf == nil {
