...
A page that has been superseded is dropped from the navigation but
kept in the repository until its content is confirmed covered by the
pages that replace it, and only then deleted. Such a page is
deliberately unreferenced, not a broken link to fix: do not relink it,
and do not delete it before the check has been made. `intro.md` and
`intro/gnss.md` are in this state now.
...
- Check that `gnss-basics.md` covers everything in the superseded
