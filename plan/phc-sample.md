...
16. **Documentation and schema for `phc.freeRunning` and `[phcsample]`.** Update `docs/man/satpulse.toml.5.md`: the main addition is explaining `phc.freeRunning` and what the free-running mode does; the `[phcsample]` section gets a brief mention that it exists and a pointer to `configs/config-schema.json` for the tunable fields rather than field-by-field man-page coverage. Extend `configs/config-schema.json` to describe `phc.freeRunning` and the full `[phcsample]` section (all the fields from `phcsample.Config`, keeping the descriptions, defaults, and bounds in sync with the Go `toml:`, `check:`, and `comment:` tags).
