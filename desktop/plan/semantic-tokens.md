...
These tokens are designed to work for both the desktop GUI and the web dashboard. The web dashboard's shared components (sky view, signal graph, property cards) map cleanly onto the same semantic names -- `surface-2` for card backgrounds, `text-primary`/`text-secondary` for data/label hierarchy, `border-subtle`/`border-strong` for SVG grid lines, `gnss-*` for constellation colours. Dashboard-specific styling (accent borders, shadows, layout) stays in dashboard code, not in shared components.
...
| `text-secondary` | zinc-600 | zinc-400 | Labels, headers |
| `text-muted` | zinc-400 | zinc-500 | Disabled, placeholder |
| `brand` | blue-600 | blue-600 | Active tabs, primary buttons |
| `brand-hover` | blue-700 | blue-500 | Hover state for brand |
...
- **Card**, **Section**, **SectionHeader** -- surface containers with borders
- **Badge** -- status indicators: `default`, `success`, `warning`, `error`
- **Table** family -- `Table`, `TableHead`, `TableBody`, `TableRow`, `Th`, `Td`; `Td` applies `font-mono` automatically for telemetry data
...
- Field labels ("Period (s)", "Survey time (s)") use `text-gray-500 dark:text-gray-400` -- also `text-secondary`
...
- Status info ("Receiver is in stationary mode") uses `text-blue-500` -- this is `brand`
