...
- `-j` gives one JSON object per edge with `t` (RFC 3339 UTC) and, for `poll`, `uncertainty: [before, after]` and `pollWidths: [start, end]` in seconds, and optional `reject`. The uncertainty reaches back to the start of the read before the edge and forward to the end of the read after it; the poll widths are those reads' durations. Timing consumers withhold edges with `reject: "acquiring"` or `reject: "anomalous"`; absent `reject` means usable, regardless of uncertainty. The command reports every catch, including rejected catches.
