...
## Unicore readback drops the QZSS L5 mask

Reading back the signal set on the UM980 reports QZSS L5 enabled while the receiver's mask list says it is masked: the `signalAliases` map in `gps/internal/unc/signal.go` has aliases for every reported mask name except `QZSSL5` (QZSS jumps from L2C to L6D), so the `$CONFIG,MASK,MASK QZSSL5*7C` response line fails the `maskSignalMap` lookup and is dropped. The fix is the missing `"QZSSL5": "Q5"` entry. Violates the readback-truth guarantee: a set that masks Q5 (acknowledged by the receiver) is contradicted by the next readback. Found by the signal probes as reported-vs-readback failures on every band-subset case involving QZSS. Evidence: run artifacts `runs/20260611-013857-ttyUSB0/098-set-signals-BDS-GAL-GLO-GPS-QZSS-L1.jsonl` (MASK Q5 acknowledged), `099-readback-...jsonl` (MASK QZSSL5 in the response, L5 reported enabled).
