...
- A fresh survey (`SurveyAgain`, `--survey` on an already-surveyed receiver) cannot be forced: the auto position is determined once, when the receiver is in the auto static mode with no previously determined position, and is then cached like the Geodetic/Cartesian slots. Verified 2026-07-13: re-issuing `setPVTMode, Static, , auto` leaves the determined position bit-identical; a Rover excursion and return recalls the cached position; only discarding position knowledge (`exeResetReceiver, Soft, PVTData` - far too destructive to ride a survey request) produced a new determination (~2.3 m from the previous one). The re-issued command is harmless, so `--survey` still acks and ensures the mode, but the position it reports is the cached determination.
