...
func parseFlags(args []string) (*flagVars, func(string) string, error) {
	...
	flags.BoolVarP(&v.tui, "tui", "t", false, "run the terminal UI instead of the web server")
	...
	if v.tui && (v.listen != "" || v.token) {
		return nil, usage, fmt.Errorf("--tui cannot be combined with --listen or --token")
	}
